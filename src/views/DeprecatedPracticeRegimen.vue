<template>
  <DataTable :value="practiceRegimen" tableStyle="min-width: 50rem">
    <template #header>
      ...
      >
        <span class="text-xl text-900 font-bold"
          ><h2>Practice Regimen</h2></span
        >
        <Button value="Notes" rounded raised
          ><i class="pi pi-clipboard"></i><span>Notes</span></Button
        >
      </div>
    </template>
    ...
    <template #footer>
      total practice time is: {{ totalMinutes(practiceRegimen) }}
    </template>
  </DataTable>
</template>
...
const deletePracticeItemFromLocalStorage = (id) => {
  const storedPracticeItems = JSON.parse(localStorage.getItem('practiceItems'))
  const updatedPracticeItems = storedPracticeItems.filter(
